...
def MSSSIM_loss(pred,label):
    MSSSIM_loss = MSSSIM(size_average=True)
    MSSSIM_loss_out = MSSSIM_loss(pred, label)
    return 1-MSSSIM_loss_out

def SSIM_loss(pred,label):
    SSIM_loss = SSIM(size_average=True)
    SSIM_loss_out = SSIM_loss(pred, label)
    return SSIM_loss_out